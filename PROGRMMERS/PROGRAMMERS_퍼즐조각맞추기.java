...
** 배운 점
퍼즐 조각과 같이 모양이 있는 물체를 인식할 때 DFS가 아니라 BFS를 사용한다.
왜냐하면 코드가 더 간단하기 때문이다. 그리고 시작점과 탐색방향으로 물체의 고유값을 만든다면 오류가 무조건 발생한다. 고유값이 같지만 모양이 다른 물체가 꼭 있다. 그렇기 때문에 시작점과의 거리 (dri, dci)값을 문자열에 append하면서 해당 물체의 고유 문자열을 만든다. 그 문자열을 key로 하는 entry의 value 값을 1 증가시키거나 1 감소시키면서 문제를 해결해나간다. 
매우 까다로운 문제였다. 이런 문제는 사실 안 나올 것으로 예상한다. (네이버/카카오 마지막 문제가 아니고서는..)
4. 코드 작성 
*/
import java.util.*;

class PROGRAMMERS_퍼즐조각맞추기 {
    // *
    int rowN, colN;
    // 게임판
    int[][] game;
    ...
    void fill() {
        ...
        for (int ri = 0; ri < rowN; ri++) {
            for (int ci = 0; ci < colN; ci++) {
                ...
                if (pieceS.getOrDefault(blank, 0) == 0)
                    continue;
